...
bool CRenderCenter::InitRender(HWND hPreview)
{
    ...
    if (!m_ShaderRGBA.InitShader(m_pDevice,
        L"shader\\texture.vs", L"shader\\texture.ps",
        sizeof(tTextureVertexType) * TEXTURE_VERTEX_COUNT, sizeof(D3DXMATRIX), 0))
    {
        return false;
    }

    if (!m_ShaderRGBA2YUV420.InitShader(m_pDevice,
        L"shader\\texture.vs", L"shader\\rgba2yuv420.ps",
        sizeof(tTextureVertexType) * TEXTURE_VERTEX_COUNT, sizeof(D3DXMATRIX), sizeof(float) * 4))
    {
        return false;
    }

    
    if (!m_ShaderYUYV422.InitShader(m_pDevice,
        L"shader\\texture.vs", L"shader\\yuv422.ps",
        sizeof(tTextureVertexType) * TEXTURE_VERTEX_COUNT, sizeof(D3DXMATRIX), sizeof(float) * 4))
    {
        return false;
    }

	if (!m_ShaderYUV420.InitShader(m_pDevice,
		L"shader\\texture.vs", L"shader\\yuv420.ps",
		sizeof(tTextureVertexType) * TEXTURE_VERTEX_COUNT, sizeof(D3DXMATRIX), 0))
	{
		return false;
	}

    if (!m_ShaderYUV420_Ex.InitShader(m_pDevice,
        L"shader\\texture.vs", L"shader\\yuv420_Ex.ps",
        sizeof(tTextureVertexType) * TEXTURE_VERTEX_COUNT, sizeof(D3DXMATRIX), 0))
    {
        return false;
    }

    if (!m_ShaderBorder.InitShader(m_pDevice,
        L"shader\\border.vs", L"shader\\border.ps",
        sizeof(tBorderVertexType) * BORDER_VERTEX_COUNT, sizeof(D3DXMATRIX), sizeof(float) *4))
    {
        return false;
    }
...
bool CRenderCenter::_InitDevice()
{
    ...
    for (UINT driverTypeIndex = 0; driverTypeIndex < numDriverTypes; driverTypeIndex++)
    {
        hr = D3D11CreateDevice(nullptr, driverTypes[driverTypeIndex], nullptr, 0, featureLevels, numFeatureLevels,
            D3D11_SDK_VERSION, m_pDevice.Assign(), &levelUsed, m_pDeviceContext.Assign());
